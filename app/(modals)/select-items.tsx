...
const ItemRow: React.FC<ItemRowProps> = React.memo(
  ({ item, qty, onChange }) => {
    ...
    useEffect(() => {
      if (qty === 0) setLocal(1);
      else if (qty !== local) setLocal(qty);
    }, [qty]); // eslint-disable-line react-hooks/exhaustive-deps
    const inc = () => {
      const n = local + 1;
      setLocal(n);
      if (qty > 0) onChange(n); // Only propagate if already added
    };
    const dec = () => {
      if (local > 1) {
        const n = local - 1;
    ...
    return (
      <View
        style={{
          ...
          borderWidth: qty ? 2 : 1,
          borderColor: qty ? theme.colors.primary : "#e2e5e8",
        }}
      >
        <View
          style={{
...
export default function SelectItemsModal() {
      ...
      (async () => {
        try {
          const [hardcoded, recent, storedSearches] = await Promise.all([
            ...
            (async () => {
              try {
                return await orderService.getRecentlyOrderedItems(8);
              } catch {
                return [];
              }
            })(),
          ...
          if (storedSearches) {
            try {
              const arr = JSON.parse(storedSearches);
  ...
  const loweredItems = useMemo(
    () => menuItems.map((m) => ({ m, nameLower: m.name.toLowerCase() })),
    [menuItems]
  );
  ...
  const sections: Section[] = useMemo(() => {
    ...
    const filtered = term
      ? loweredItems
          .filter((obj) => obj.nameLower.includes(term))
          .map((obj) => obj.m)
      : menuItems;
    if (category !== "All") {
    ...
    if (!term && recentItems.length) {
      ...
      for (const r of recentItems) {
        if (!seen.has(r.id) && filtered.find((f) => f.id === r.id)) {
          seen.add(r.id);
          recentsFiltered.push(r);
        }
      }
      if (recentsFiltered.length)
        ordered.push({ title: "Quick", data: recentsFiltered.slice(0, 8) });
    }
    CATEGORIES.forEach((c) => {
      if (group[c.name]) ordered.push({ title: c.name, data: group[c.name] });
    });
    Object.keys(group)
      .filter((k) => !CATEGORIES.find((c) => c.name === k))
      .sort()
      .forEach((k) => ordered.push({ title: k, data: group[k] }));
    return ordered;
  }, [menuItems, loweredItems, debouncedSearch, category, recentItems]);
  ...
  const renderItemFn = ({ item }: { item: MenuItem }) => (
    <ItemRow
      item={item}
      qty={getQty(item.id)}
      onChange={(q: number) => setQty(item, q)}
    />
  );
      ...
      <View style={{ flex: 1 }}>
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderWidth: 1,
                    borderColor: active ? theme.colors.primary : "#e3e5e8",
                    alignItems: "center",
                    justifyContent: "center",
                    marginRight: 8,
                    position: "relative",
        ...
        <View style={{ flex: 1 }}>
          {loading ? (
            ...
            >
              ...
              >
                Menu will appear here once items are added.
              </Text>
            </View>
          ) : sections.length === 0 ? (
            <View
        ...
        {order$.selectedItems.length > 0 && (
          ...
          >
            ...
            <TouchableOpacity
              onPress={() => setShowSelectedSheet(true)}
              style={{
        ...
        {showSelectedSheet && (
          ...
          >
            ...
            <TouchableOpacity
              activeOpacity={1}
              onPress={() => setShowSelectedSheet(false)}
              style={{
                position: "absolute",
                left: 0,
                right: 0,
              ...
              >
                {order$.selectedItems.map((si) => (
                  ...
                  >
                    ...
                    >
                      <TouchableOpacity
                        onPress={() => setQty(si.item, si.quantity - 1)}
                        style={{
                          width: 26,
                          height: 26,
                          borderRadius: 13,
