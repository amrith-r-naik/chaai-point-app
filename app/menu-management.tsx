  ...
  >
    ...
    >
      <Text style={{ fontSize: 20 }}>
        {getCategoryEmoji(item.category || undefined)}
      </Text>
    </View>
...
export default function MenuManagementScreen() {
  ...
  const lowered = useMemo(
    () => items.map((m) => ({ m, nameLower: m.name.toLowerCase() })),
    [items]
  );
  const sections: Section[] = useMemo(() => {
    const term = search.trim().toLowerCase();
    const filtered = term
      ? lowered.filter((o) => o.nameLower.includes(term)).map((o) => o.m)
      : items;
    if (category !== "All") {
      const data = filtered.filter(
        (f) => (f.category || "Uncategorized") === category
      );
  ...
  const validate = (): string | null => {
    ...
    const p = parseFloat(menuForm.price);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      else await menuService.createMenuItem(payload);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <View
        style={{ height: topInset, backgroundColor: theme.colors.primary }}
      />
          ...
          <ScrollView style={{ flex: 1, padding: 16 }}>
            ...
            <View style={{ marginBottom: 20 }}>
              ...
              >
                {CATEGORIES.map((c) => {
                  const active =
                    !showCustomCategory && menuForm.category === c.name;
                  return (
                    <TouchableOpacity
                      key={c.name}
                      onPress={() => {
                        setMenuForm({ ...menuForm, category: c.name });
                        setShowCustomCategory(false);
                      }}
                      style={{
                        paddingHorizontal: 12,
                ...
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    height: 34,
                    borderRadius: 17,
                    backgroundColor: showCustomCategory
                      ? theme.colors.primary
                      : "#f3f4f6",
                    borderWidth: 1,
                    borderColor: showCustomCategory
                      ? theme.colors.primary
                      : "#e5e7eb",
                    alignItems: "center",
                    justifyContent: "center",
                ...
                >
                  ...
                  <Text
                    style={{
                      fontSize: 14,
                      color: showCustomCategory ? "white" : theme.colors.text,
                      fontWeight: showCustomCategory ? "700" : "500",
                    }}
                  >
                    New
                  </Text>
                ...
                <View style={{ marginTop: 10 }}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    <TextInput
                      ...
                      onChangeText={(text: string) =>
                        setMenuForm({ ...menuForm, category: text })
                      }
