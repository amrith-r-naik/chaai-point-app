...
class DueService {
  ...
  private async getNextBillNumber(): Promise<number> {
    ...
    const result = await db.getFirstAsync(`
      SELECT MAX(billNumber) as maxBill FROM bills
    `) as any;

    return (result?.maxBill || 0) + 1;
  }
  ...
  private async getNextReceiptNumber(): Promise<number> {
    ...
    const result = await db.getFirstAsync(`
      SELECT MAX(receiptNo) as maxReceipt FROM receipts
    `) as any;

    return (result?.maxReceipt || 0) + 1;
  }
